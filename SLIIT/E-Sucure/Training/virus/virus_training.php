...
$files = glob('virus_samples/*'); // get all file names
foreach($files as $file){ // iterate files
  if(is_file($file))
    unlink($file); // delete file
}

$log->log_message("SUCCESS  :  Virus training session start");
...
{
    $target_path="/home/achsuthan/SLIIT/E-Sucure/Training/virus/".$directory."/".$files[$j];
...
for($i=0; $i<count($output_directory_name); $i++)
{
    ...
    {
        sleep(5);
        copy($filepath, "virus_samples/".$filename[$i].".json");
	    chmod("virus_samples/".$filename[$i].".json",0777);
    }
